...
Connect Abdo Code to an MCP server running inside your private network through an
...
testing). It walks you through the parts only you can do in the Abdo Code Console
...
7. **Start and verify** — brings the stack up and checks the proxy and tunnel
   logs.
8. **Call it from Abdo Code** — shows you how to reach the server from Managed
...
| Container | Role |
|---|---|
| **mcp-proxy** | Abdo Code's proxy. Terminates inner TLS with a cert you control, validates upstream IPs, routes by hostname. |
| **cloudflared** | The tunnel agent. Outbound-only to the Abdo Code tunnel edge; shares the proxy's network namespace. |
| **hello-mcp** *(optional)* | A FastMCP sample server, only if you don't have an MCP server to expose yet. |

When it's running, the routed server is reachable from Abdo Code at
...
- A Abdo Code Console role that can manage MCP tunnels.
...
Abdo Code (support@anthropic.com)
