...
---
<!-- Modified by TechnologyKSA for Abdo Code — origin: anthropics/knowledge-work-plugins@1f517b9de47e skills/job-post-builder/SKILL.md (Apache-2.0). Names, paths and tool references adapted; see NOTICE.md. -->
# Job Post Builder
...
to DocuSign via Abdo Code in Chrome.
...
5. **Assemble offer letter** — Build offer letter with bracketed placeholders using
   ...
   docx skill.
6. **Route to DocuSign (if requested)** — Use Abdo Code in Chrome to navigate DocuSign,
...
- **Never send a DocuSign envelope without approval.** Save the envelope as a draft
  and return the URL. The user must review and confirm before Abdo Code clicks Send.
...
Use Abdo Code in Chrome to upload the offer letter into DocuSign and set up the
