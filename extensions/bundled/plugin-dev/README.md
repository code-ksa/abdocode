...
A comprehensive toolkit for developing Abdo Code plugins with expert guidance on hooks, MCP integration, plugin structure, and marketplace publishing.

## Overview

The plugin-dev toolkit provides seven specialized skills to help you build high-quality Abdo Code plugins:
...
7. **Testing** - Verify plugin works in Abdo Code
...
**Trigger phrases:** "create a hook", "add a PreToolUse hook", "validate tool use", "implement prompt-based hooks", "${extension}", "block dangerous commands"
...
- Environment variable expansion (${extension}, user vars)
...
- AI-assisted agent generation using Abdo Code's proven prompt
...
- 3 reference docs: agent-creation-system-prompt (from Abdo Code), system-prompt-design (~4,000w), triggering-examples (~2,500w)
...
- Based on skill-creator methodology adapted for Abdo Code plugins
...
This keeps Abdo Code's context focused while providing deep knowledge when needed.
...
- Based on official Abdo Code documentation
