...
Complete reference for all MCP server types supported in Abdo Code plugins.
...
{
  "my-server": {
    "command": "${extension}/servers/custom-server",
...
1. **Startup**: Abdo Code spawns process with `command` and `args`
2. **Communication**: JSON-RPC messages via stdin/stdout
3. **Lifecycle**: Process runs for entire Abdo Code session
4. **Shutdown**: Process terminated when Abdo Code exits
...
{
  "custom": {
    "command": "${extension}/servers/my-server.js",
    "args": ["--verbose"]
  }
}
```
...
1. **Use absolute paths or ${extension}**
