...
Complete guide to authentication methods for MCP servers in Abdo Code plugins.
...
Abdo Code automatically handles the complete OAuth 2.0 flow for SSE and HTTP servers:

1. User attempts to use MCP tool
2. Abdo Code detects authentication needed
3. Opens browser for OAuth consent
4. User authorizes in browser
5. Tokens stored securely by Abdo Code
...
No additional auth configuration needed! Abdo Code handles everything.
...
Tokens are stored securely by Abdo Code:
...
```json
{
  "api": {
    ...
    "headersHelper": "${extension}/scripts/get-headers.sh"
  }
}
```
...
{
  "secure-api": {
    "command": "${extension}/servers/mtls-wrapper",
...
```json
{
  "headersHelper": "${extension}/scripts/generate-jwt.sh"
}
```
