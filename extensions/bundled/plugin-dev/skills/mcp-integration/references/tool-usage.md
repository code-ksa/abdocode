...
Complete guide to using MCP tools effectively in Abdo Code plugin commands and agents.

## Overview

Once an MCP server is configured, its tools become available with the prefix `mcp__plugin_<plugin-name>_<server-name>__<tool-name>`. Use these tools in commands and agents just like built-in Abdo Code tools.
...
- Can use any tool Abdo Code determines is necessary
...
Abdo Code automatically structures tool calls based on schema:

```typescript
// Abdo Code generates this internally
{
...
```markdown
Steps:
1. Make parallel calls (Abdo Code handles this automatically):
