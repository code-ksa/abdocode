...
head -n 20 .abdo/commands/my-command.md | grep -A 10 "^---"

# Check for closing frontmatter marker
head -n 20 .abdo/commands/my-command.md | grep -c "^---" # Should be 2

# Verify file has .md extension
ls .abdo/commands/*.md

# Check file is in correct location
test -f .abdo/commands/my-command.md && echo "Found" || echo "Missing"
```
...
cat > .abdo/commands/test-bash.md << 'EOF'
---
...
cat > .abdo/commands/test-forbidden.md << 'EOF'
---
...
# Command: .abdo/commands/risky-operation.md
...
COMMANDS_CHANGED=$(git diff --cached --name-only | grep "\.abdo/commands/.*\.md")
  ...
  test:
    ...
    steps:
      ...
      - name: Validate command structure
        run: |
          for cmd in .abdo/commands/*.md; do
            echo "Testing: $cmd"
      ...
      - name: Check for TODOs
        run: |
          if grep -r "TODO" .abdo/commands/; then
...
```bash
# Monitor Abdo Code during command execution
# In terminal 1:
claude --debug
...
```bash
# Check file location
ls -la .abdo/commands/my-command.md

# Check permissions
chmod 644 .abdo/commands/my-command.md

# Check syntax
head -n 20 .abdo/commands/my-command.md
...
```bash
# Verify syntax
grep '\$1' .abdo/commands/my-command.md
grep '\$ARGUMENTS' .abdo/commands/my-command.md

# Test with simple command first
echo "Test: \$1 and \$2" > .abdo/commands/test-args.md
```
...
```bash
# Check allowed-tools
grep "allowed-tools" .abdo/commands/my-command.md

# Verify command syntax
grep '!\`' .abdo/commands/my-command.md
