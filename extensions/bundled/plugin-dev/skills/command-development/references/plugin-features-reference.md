...
This reference covers features and patterns specific to commands bundled in Abdo Code plugins.
...
Abdo Code automatically discovers commands in plugins using the following locations:
...
`${extension}` is a special environment variable available in plugin commands that resolves to the absolute path of the plugin directory.
...
Run analysis: !`node ${extension}/scripts/analyze.js`

Read template: @${extension}/templates/report.md
```
...
Lint results: !`node ${extension}/bin/lint.js $1`
...
Configuration: @${extension}/config/deploy-config.json
...
Use this template: @${extension}/templates/api-report.md
...
Step 1 - Prepare: !`bash ${extension}/scripts/prepare.sh $1`
Step 2 - Config: @${extension}/config/$1.json
Step 3 - Execute: !`${extension}/bin/execute $1`
...
2. **Validate file existence:**
   ...
   !`test -f ${extension}/config.json && echo "exists" || echo "missing"`

   If config exists, load it: @${extension}/config.json
...
4. **Combine with arguments:**
   ```markdown
   Run: !`${extension}/bin/process.sh $1 $2`
   ```
...
- Verify syntax is exact: `${extension}`
...
Load configuration: @${extension}/deploy-config.json
...
Template: @${extension}/templates/component-docs.md
...
Validate: !`bash ${extension}/scripts/validate.sh`
...
Environment config: @${extension}/config/$1.json
...
Cache directory: ${extension}/cache/

Analyze @$1 and save results to cache:
!`mkdir -p ${extension}/cache && date > ${extension}/cache/last-run.txt`
...
Commands can trigger plugin agents using the `delegate` tool:
...
Note: This uses the `delegate` tool to launch the plugin's code-analyzer agent.
```

**Key points:**
- Agent must be defined in plugin's `agents/` directory
- Abdo Code will automatically use Task tool to launch agent
...
- Mention skill by name to hint Abdo Code should invoke it
...
- Config exists: !`test -f ${extension}/config.json && echo "✓" || echo "✗"`
- Scripts exist: !`test -d ${extension}/scripts && echo "✓" || echo "✗"`
- Tools available: !`test -x ${extension}/bin/analyze && echo "✓" || echo "✗"`
...
Try processing: !`node ${extension}/scripts/process.js $1 2>&1 || echo "ERROR: $?"`
...
1. **Use ${extension} for all plugin-internal paths**
