...
Practical examples of commands designed for Abdo Code plugins, demonstrating plugin-specific patterns and features.
...
!`node ${extension}/scripts/quality-check.js $1`
...
**Key features:**
- Uses `${extension}` for portable path
...
**Security scan:**
!`bash ${extension}/scripts/security-scan.sh $1`

**Performance analysis:**
!`bash ${extension}/scripts/perf-analyze.sh $1`

**Best practices check:**
!`bash ${extension}/scripts/best-practices.sh $1`
...
Template structure: @${extension}/templates/api-documentation.md
...
!`bash ${extension}/scripts/pre-release-check.sh $1`

**Step 2 - Build artifacts:**
!`bash ${extension}/scripts/build-release.sh $1`

**Step 3 - Run test suite:**
!`bash ${extension}/scripts/run-tests.sh`

**Step 4 - Package release:**
!`bash ${extension}/scripts/package.sh $1`
...
Deployment configuration for $1: @${extension}/config/$1-deploy.json
...
- Plugin's linting rules: ${extension}/config/lint-rules.json
- Security checklist: ${extension}/checklists/security.md
- Performance guidelines: ${extension}/docs/performance.md

Note: This uses the `delegate` tool to launch the plugin's code-reviewer agent for thorough analysis.
```
...
Run plugin analyzer: !`node ${extension}/scripts/analyze.js $1`
...
Template: @${extension}/templates/review-report.md
...
Check build script exists: !`test -x ${extension}/scripts/build.sh && echo "EXISTS" || echo "MISSING"`

Verify configuration available: !`test -f ${extension}/config/$1.json && echo "FOUND" || echo "NOT_FOUND"`

If all validations pass:

**Configuration:** @${extension}/config/$1.json

**Execute build:** !`bash ${extension}/scripts/build.sh $1 2>&1`

**Validation results:** !`bash ${extension}/scripts/validate-build.sh $1 2>&1`
...
Load environment configuration: @${extension}/config/$1-checks.json

Determine check level: !`echo "$1" | grep -E "^prod$" && echo "FULL" || echo "BASIC"`

**For production environment:**
- Full test suite: !`bash ${extension}/scripts/test-full.sh`
- Security scan: !`bash ${extension}/scripts/security-scan.sh`
- Performance audit: !`bash ${extension}/scripts/perf-check.sh`
- Compliance check: !`bash ${extension}/scripts/compliance.sh`

**For non-production environments:**
- Basic tests: !`bash ${extension}/scripts/test-basic.sh`
- Quick lint: !`bash ${extension}/scripts/lint.sh`
...
```markdown
Check exists: !`test -f ${extension}/path/file && echo "YES" || echo "NO"`
```
...
2. **Verify ${extension} expansion:**
   ```bash
   # Add debug output to command
   !`echo "Plugin root: ${extension}"`
   ```
...
4. **Validate resource availability:**
   ...
   !`ls -la ${extension}/config/`
   ```

### Common Mistakes to Avoid

1. **Using relative paths instead of ${extension}:**
   ```markdown
   # Wrong
   !`node ./scripts/analyze.js`

   # Correct
   !`node ${extension}/scripts/analyze.js`
   ```
...
4. **Hardcoding plugin paths:**
   ...
   @/home/user/.abdo/plugins/my-plugin/config.json
