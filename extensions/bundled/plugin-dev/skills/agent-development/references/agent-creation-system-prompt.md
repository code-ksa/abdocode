...
**You send to Abdo Code with the system prompt above:**
```
Create an agent configuration based on this request: "I need an agent that reviews pull requests for code quality issues"
```

**Abdo Code returns JSON (note: prose `whenToUse`, "When to invoke" section in `systemPrompt`):**
```json
{
...
- Follow project documentation standards from ABDO.md
```
...
The prompt specifically mentions using ABDO.md context:
...
2. Feed to Abdo Code with this system prompt
