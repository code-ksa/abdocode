...
description: This skill should be used when the user asks to "create an agent", "add an agent", "write a subagent", "agent frontmatter", "when to use description", "agent examples", "agent tools", "agent colors", "autonomous agent", or needs guidance on agent structure, system prompts, triggering conditions, or agent development best practices for Abdo Code plugins.
---
<!-- Modified by TechnologyKSA for Abdo Code — origin: anthropics/claude-plugins-official@85cce0381e78 skills/agent-development/SKILL.md (Apache-2.0). Names, paths and tool references adapted; see NOTICE.md. -->
# Agent Development for Abdo Code Plugins
...
Defines when Abdo Code should trigger this agent. **This is the most critical field** — it is loaded into context whenever the agent is registered, so the harness can decide when to dispatch.
...
- `sonnet` - Abdo Code Sonnet (balanced)
- `opus` - Abdo Code Opus (most capable, expensive)
- `haiku` - Abdo Code Haiku (fast, cheap)
...
Use this prompt pattern (extracted from Abdo Code):
...
3. Check Abdo Code loads the agent
...
- **`references/agent-creation-system-prompt.md`** - The exact prompt from Abdo Code
