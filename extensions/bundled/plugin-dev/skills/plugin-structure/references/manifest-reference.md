...
The manifest MUST be in the `.claude-plugin/` directory at the plugin root. Abdo Code will not recognize plugins without this file in the correct location.
...
- Plugin identification in Abdo Code
...
```json
{
  "mcpServers": {
    "github": {
      "command": "node",
      "args": ["${extension}/servers/github-mcp.js"],
      "env": {
...
When Abdo Code loads components:
...
Abdo Code validates the manifest on plugin load:
