...
4. **Register components**: Make available to Abdo Code
5. **Initialize**: Start MCP servers, register hooks

**Timing**: Component registration happens during Abdo Code initialization, not continuously.

### Activation Phase

When components are used:

**Commands**: User types slash command → Abdo Code looks up → Executes
**Agents**: Task arrives → Abdo Code evaluates capabilities → Selects agent
**Skills**: Task context matches description → Abdo Code loads skill
**Hooks**: Event occurs → Abdo Code calls matching hooks
...
**Note**: Abdo Code doesn't support nested command discovery automatically. Use custom paths:
...
**Note**: Use build script to combine files, Abdo Code doesn't support file references.
...
```bash
#!/bin/bash
source "${extension}/lib/test-utils.sh"
run_tests
```
