...
description: Two-tier memory system that makes Abdo Code a true workplace collaborator. Decodes shorthand, acronyms, nicknames, and internal language so Abdo Code understands requests like a colleague would. ABDO.md for working memory, memory/ directory for the full knowledge base.
---
<!-- Modified by TechnologyKSA for Abdo Code — origin: anthropics/knowledge-work-plugins@1f517b9de47e skills/memory-management/SKILL.md (Apache-2.0). Names, paths and tool references adapted; see NOTICE.md. -->
# Memory Management

Memory makes Abdo Code your workplace collaborator - someone who speaks your internal language.
...
Without memory, that request is meaningless. With memory, Abdo Code knows:
...
```
ABDO.md          ← Hot cache (~30 people, common terms)
memory/
...
- Searched when something isn't in ABDO.md
...
This tiered approach keeps ABDO.md lean (~100 lines) while supporting unlimited scale in memory/.

## File Locations

- **Working memory:** `ABDO.md` in current working directory
- **Deep memory:** `memory/` subdirectory

## Working Memory Format (ABDO.md)
...
```
1. ABDO.md (hot cache)     → Check first, covers 90% of cases
...
Now Abdo Code can act with full context.
```
...
- **Bold** terms in ABDO.md for scannability
- Keep ABDO.md under ~100 lines (the "hot 30" rule)
...
- When something's used frequently, promote it to ABDO.md
- When something goes stale, demote it to memory/ only

## What Goes Where

| Type | ABDO.md (Hot Cache) | memory/ (Full Storage) |
|------|----------------------|------------------------|
...
This keeps ABDO.md fresh and relevant.
