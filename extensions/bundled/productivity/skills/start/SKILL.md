...
---
<!-- Modified by TechnologyKSA for Abdo Code — origin: anthropics/knowledge-work-plugins@1f517b9de47e skills/start/SKILL.md (Apache-2.0). Names, paths and tool references adapted; see NOTICE.md. -->
# Start Command
...
**If `dashboard.html` doesn't exist:** Copy it from `${extension}/skills/dashboard.html` to the current working directory.

**If `ABDO.md` and `memory/` don't exist:** This is a fresh setup — after opening the dashboard, begin the memory bootstrap workflow (see below). Place these in the current working directory.
...
Only do this if `ABDO.md` and `memory/` don't exist yet.
...
**ABDO.md** (working memory, ~50-80 lines):
```markdown
# Memory
