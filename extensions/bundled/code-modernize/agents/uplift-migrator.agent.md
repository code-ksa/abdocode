...
description: Migrates ONE project/module of an in-flight same-stack version uplift by applying a proven pilot playbook — minimal diff, then runs that unit's real build to prove it. Refuses to migrate anything if no playbook exists ye
tools: read, glob, grep, write, edit, run
---
<!-- Modified by TechnologyKSA for Abdo Code — origin: anthropics/claude-plugins-official@85cce0381e78 agents/uplift-migrator.md (Apache-2.0). Names, paths and tool references adapted; see NOTICE.md. -->
