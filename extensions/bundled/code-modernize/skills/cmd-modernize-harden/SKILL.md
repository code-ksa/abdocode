---
name: cmd-modernize-harden
description: Security vulnerability scan with a reviewable remediation patch — OWASP, CWE, CVE, secrets, injection
---
<!-- Modified by TechnologyKSA for Abdo Code — origin: anthropics/claude-plugins-official@85cce0381e78 commands/modernize-harden.md (Apache-2.0). Names, paths and tool references adapted; see NOTICE.md. -->
...
(referred to as `$1` below) is the first non-flag token in `(the user's request)`;
...
```
Workflow({
  scriptPath: "${extension}/workflows/harden-scan.js",
  args: { system: "$1" }
})
```
...
**Fallback — direct subagent** (older Abdo Code builds without the
