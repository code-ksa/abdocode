...
When enabled, this plugin automatically adds instructions at the start of each session that encourage Abdo Code to:
...
Instead of implementing everything automatically, Abdo Code will:
...
The plugin uses a SessionStart hook to inject additional context into every session. This context instructs Abdo Code to adopt an interactive teaching approach where you actively participate in writing key parts of the code.

## When Abdo Code requests contributions

Abdo Code will ask you to write code for:
...
## When Abdo Code won't request contributions

Abdo Code will implement directly:
...
**Abdo Code:** I've set up the authentication middleware. The session timeout behavior is a security vs. UX trade-off - should sessions auto-extend on activity, or have a hard timeout?
...
In addition to interactive learning, Abdo Code will provide educational insights about implementation choices using this format:
...
This SessionStart hook pattern is roughly equivalent to ABDO.md, but it is more flexible and allows for distribution through plugins.
...
Learning by doing is more effective than passive observation. This plugin transforms your interaction with Abdo Code from "watch and learn" to "build and understand," ensuring you develop practical skills through hands-on coding of meaningful logic.
