---
name: m5-onboard
description: End-to-end onboarding for a freshly-plugged-in M5Stack ESP32 device (Cardputer, Cardputer-Adv, Core, CoreS3, Stick) — detect on USB, flash UIFlow 2.0 firmware, and install the Abdo Code Buddy MicroPython app bundle. Use whenever the user plugs in or wants to flash/provision/reset an M5Stack or ESP32 board, or says "m5-onboard go".
---
<!-- Modified by TechnologyKSA for Abdo Code — origin: anthropics/claude-plugins-official@85cce0381e78 skills/m5-onboard/SKILL.md (Apache-2.0). Names, paths and tool references adapted; see NOTICE.md. -->
# M5Stack Onboarding

This skill automates the full cold-start workflow for an M5Stack ESP32 device: detect on USB, identify model, flash UIFlow 2.0, and push a MicroPython app bundle onto `/flash/` so the device boots into user software. The apps we ship (Abdo Code Buddy, Snake, Hello) talk over BLE or USB. The workflow runs on macOS, Linux, and Windows; the skill was developed against an M5Stack Basic v2.6 (CH9102 bridge, ESP32-D0WDQ6-V3, 16 MB flash) and generalized to cover the rest of the Core family, with the Cardputer-Adv (ESP32-S3, native USB) as the current default target.
...
Users running `onboard.py` directly in their own terminal (not via Abdo Code) will see all output live — no changes needed there.
...
The known apps name `buddy` resolves to the `buddy/device/` directory in this repo (custom launcher + Hello + Abdo Code Buddy BLE client + Snake). Any other `--apps` value is treated as a filesystem path.
...
- **Abdo Code Buddy over BLE.** First time only: in Abdo Code Desktop, **Help → Troubleshooting → Enable Developer Tools** (one-time, persists across launches). Then **Developer menu → Hardware Buddy → Connect**. BLE works regardless of the WiFi state — the link to Abdo Code.app is local.
...
Python itself has to exist before this skill can do anything — you can't bootstrap an interpreter from inside one. `git` is **not** required — the `/maker-setup` command falls back to downloading the GitHub tarball with `curl`+`tar` (both pre-installed on macOS, Linux, and Windows 10+) when `git --version` fails. Abdo Code's responsible for detecting Python and installing it if missing *before* running any `scripts/*.py` invocation. Detection is just running `python3 --version` / `python --version` — if it fails, Abdo Code fetches Python with the host's native package manager before anything else.

**Per-OS Python bootstrap (Abdo Code's responsibility if missing):**
...
- **Bundle path resolution.** `install_apps.py`'s `--src buddy` shorthand resolves in this order:
  ...
  2. The `buddy/device/` directory inside this repo, found via `os.path.realpath(__file__)` walking up from `install_apps.py`. Works for any clone location, including symlinked skill installs at `~/.abdo/skills/m5-onboard/`.
