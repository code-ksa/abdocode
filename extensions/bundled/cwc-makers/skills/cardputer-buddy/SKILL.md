...
description: Iterate on the Cardputer-Adv MicroPython app bundle (Abdo Code Buddy, Snake, Hello) after the device is already provisioned via m5-onboard. Use when the user wants to add a new app, push a single changed .py without re-flashing, watch device serial logs, or run a one-shot REPL command. Trigger on "add an app", "push to the cardputer", "tail the device", "run on the device", or follow-up work after /maker-setup.
---
<!-- Modified by TechnologyKSA for Abdo Code — origin: anthropics/claude-plugins-official@85cce0381e78 skills/cardputer-buddy/SKILL.md (Apache-2.0). Names, paths and tool references adapted; see NOTICE.md. -->
...
└── apps/
    ├── claude_buddy.py  BLE client → Abdo Code Desktop's Hardware Buddy
    ├── hello_cardputer.py
    └── snake.py
```
