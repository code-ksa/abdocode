...
Plug in your M5Stack Cardputer-Adv over USB-C, type `/maker-setup`, and Abdo Code will:

1. Clone [`moremas/build-with-claude`](https://github.com/moremas/build-with-claude)
2. Detect the device, flash UIFlow 2.0 firmware, and install the Abdo Code Buddy + Hello + Snake app bundle
3. Walk you through the one physical step (the download-mode button press on the back of the device)
4. Hand you a working pocket computer that pairs with Abdo Code Desktop over BLE

Then ask Abdo Code to build whatever you want next — a magic 8-ball, a pixel pet, a weather ticker — and it'll write the MicroPython and push it to the device without re-flashing.
...
`/maker-setup` is the intended entry point; the skills are also auto-triggered by Abdo Code when relevant. Skill content is vendored from the upstream repo so Abdo Code has the domain knowledge in-context without symlinking anything into `~/.abdo/skills/`.
