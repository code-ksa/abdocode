...
description: Analyze a codebase and recommend Abdo Code automations (hooks, subagents, skills, plugins, MCP servers). Use when user asks for automation recommendations, wants to optimize their Abdo Code setup, mentions improving Abdo Code workflows, asks how to first set up Abdo Code for a project, or wants to know what Abdo Code features they should use.
---
<!-- Modified by TechnologyKSA for Abdo Code — origin: anthropics/claude-plugins-official@85cce0381e78 skills/claude-automation-recommender/SKILL.md (Apache-2.0). Names, paths and tool references adapted; see NOTICE.md. -->
# Abdo Code Automation Recommender

Analyze codebase patterns to recommend tailored Abdo Code automations across all extensibility options.

**This skill is read-only.** It analyzes the codebase and outputs recommendations. It does NOT create or modify any files. Users implement the recommendations themselves or ask Abdo Code separately to help build them.
...
| **Skills** | Packaged expertise, workflows, and repeatable tasks (invoked by Abdo Code or user via `/skill-name`) |
...
# Check for existing Abdo Code config
ls -la .claude/ ABDO.md 2>/dev/null
...
Create skills in `.abdo/skills/<name>/SKILL.md`. Some are also available via plugins:
...
| Code style | **project-conventions** | Abdo Code-only |
...
```markdown
## Abdo Code Automation Recommendations
...
#### [skill name]
**Why**: [specific reason]
**Create**: `.abdo/skills/[name]/SKILL.md`
**Invocation**: User-only / Both / Abdo Code-only
...
- `user-invocable: false` — Abdo Code-only (for background knowledge)
...
Recommend headless Abdo Code for automated pipelines:
...
Configure allowed tools in `.abdo/settings.json`:
