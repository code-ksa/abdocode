...
Skills are packaged expertise with workflows, reference materials, and best practices. Create them in `.abdo/skills/<name>/SKILL.md`. Skills can be invoked by Abdo Code automatically when relevant, or by users directly with `/skill-name`.
...
Create project-specific skills in `.abdo/skills/<name>/SKILL.md`.
...
user-invocable: false           # Only Abdo Code can invoke (for background knowledge)
...
| Setting | User | Abdo Code | Use for |
|---------|------|--------|---------|
...
3. Run validation: `bash ~/.abdo/skills/create-migration/scripts/validate-migration.sh`
...
### Project Conventions (Abdo Code-only)

Background knowledge Abdo Code applies automatically:
...
The command output replaces the placeholder before Abdo Code sees the skill content.
