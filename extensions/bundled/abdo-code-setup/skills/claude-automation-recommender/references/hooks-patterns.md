...
Hooks automatically run commands in response to Abdo Code events. They're ideal for enforcement and automation that should happen consistently.
...
Notification hooks run when Abdo Code sends notifications. Use matchers to filter by notification type.

### Permission Alerts
| Matcher | Use Case |
|---------|----------|
| `permission_prompt` | Alert when Abdo Code requests permissions |
...
| Matcher | Use Case |
|---------|----------|
| `idle_prompt` | Alert when Abdo Code is waiting for input (60+ seconds idle) |

**Recommend**: Play sound or send notification when Abdo Code needs attention
**Value**: Know when Abdo Code is ready for your input
    ...
    "Notification": [
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "osascript -e 'display notification \"Abdo Code is waiting\" with title \"Abdo Code\"'"
          }
        ]
      }
    ]
...
|---------|---------------|
| `permission_prompt` | Abdo Code needs permission for a tool |
| `idle_prompt` | Abdo Code waiting for input (60+ seconds) |
...
Hooks go in `.abdo/settings.json`:
