...
MCP (Model Context Protocol) servers extend Abdo Code's capabilities by connecting to external tools and services.
...
2. **Global config** (`~/.abdo.json`) - Available across all projects
...
### context7
**Best for**: Projects using popular libraries/SDKs where you want Abdo Code to code with up-to-date documentation
...
**Value**: Abdo Code fetches live documentation instead of relying on training data, reducing hallucinated APIs and outdated patterns.
...
**Value**: Abdo Code can interact with your running app, take screenshots, fill forms, and verify UI behavior.
...
**Value**: Abdo Code can query tables, manage auth, and interact with Supabase storage directly.
...
**Value**: Abdo Code can introspect the live deployment (tables, function specs, env vars, logs) and execute queries/mutations against it via tools like `tables`, `function-spec`, `data`, `run-once-query`, `logs`, `env list/set/get`. Run via `npx convex mcp start`.
...
**Value**: Abdo Code can create issues, review PRs, check workflow runs, and manage releases.
...
**Value**: Abdo Code can investigate Sentry issues, find root causes, and suggest fixes.
...
**Value**: Abdo Code remembers project context, decisions, and patterns across conversations.
...
| `@anthropic-ai/sdk` | context7 for Abdo Code docs |
