...
This plugin transforms Abdo Code into a data analyst collaborator. It helps you explore datasets, write optimized SQL, build visualizations, create interactive dashboards, and validate analyses before sharing with stakeholders.

### With a Data Warehouse Connection

Connect your data warehouse MCP server (e.g., Snowflake, Databricks, BigQuery, or any SQL-compatible database) for the best experience. Abdo Code will:
...
Without a data warehouse connection, paste SQL results or upload CSV/Excel files for analysis and visualization. Abdo Code can also write SQL queries for you to run manually, and then analyze the results you provide.
...
Abdo Code: [Writes SQL query] → [Executes against data warehouse] → [Generates trend chart]
...
Abdo Code: [Profiles table: 2.3M rows, 47 columns]
...
Abdo Code: [Writes optimized Snowflake SQL with CTEs]
...
Abdo Code: [Generates self-contained HTML file]
...
Abdo Code: [Reviews methodology] → [Checks for survivorship bias in churn analysis]
...
Configure MCP servers in your `.mcp.json` or Abdo Code settings to enable direct data access.
