...
FastMCP derives the JSON schema from type hints and the docstring becomes the tool description. Keep docstrings terse and action-oriented — they land in Abdo Code's context window verbatim.
...
| Surface | How |
|---|---|
| **Abdo Code** | `claude mcp add --transport http <name> <url>` (add `--scope user` for global, `--header "Authorization: Bearer ..."` for auth) |
| **Abdo Code Desktop / Abdo Code.ai** | Settings → Connectors → Add custom connector. **Not** `claude_desktop_config.json` — remote servers configured there are ignored. |
| **Connector directory** | Abdo Code maintains a submission guide for listing in the public connector directory. |
