...
ID. Traffic from claude.ai is proxied through Abdo Code's egress — every web
...
Abdo Code Desktop, Abdo Code, and other hosts connect **directly from the
...
the aggregate Abdo Code pool. If true per-user limits matter, that's the
...
`X-Forwarded-For: 160.79.108.42` and claim the Abdo Code tier. Set it to the
exact number of trusted hops (e.g. `1` behind a single LB, `2` behind
Cloudflare → origin LB) and **never `true` in production**.

## Hard-allowlisting Abdo Code IPs is a product decision

Blocking everything outside `160.79.104.0/21` locks out Desktop, Abdo Code,
