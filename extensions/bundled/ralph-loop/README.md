...
Implementation of the Ralph Wiggum technique for iterative, self-referential AI development loops in Abdo Code.
...
This plugin implements Ralph using a **Stop hook** that intercepts Abdo Code's exit attempts:

```bash
# You run ONCE:
/ralph-loop "Your task description" --completion-promise "DONE"

# Then Abdo Code automatically:
# 1. Works on the task
# 2. Tries to exit
...
- Abdo Code's previous work persists in files
- Each iteration sees modified files and git history
- Abdo Code autonomously improves by reading its own past work in files
...
```json
"command": "\"C:/Program Files/Git/bin/bash.exe\" ${extension}/hooks/stop-hook.sh"
```

**Location**: `~/.abdo/plugins/cache/claude-plugins-official/ralph-wiggum/<hash>/hooks/hooks.json`
...
Run `/help` in Abdo Code for detailed command reference and examples.
