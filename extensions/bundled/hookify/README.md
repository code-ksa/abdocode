...
Ask Abdo Code to run a command that should trigger the rule:
```
Run rm -rf /tmp/test
```
...
- **`stop`**: Triggers when Abdo Code wants to stop (for completion checks)
...
**This rule blocks the operation** - Abdo Code will not be allowed to execute these commands.
...
**This rule warns but allows** - Abdo Code sees the message but can still proceed.
...
**This blocks Abdo Code from stopping** if no test commands appear in the session transcript. Enable only when you want strict enforcement.
...
This plugin is part of the Abdo Code Marketplace. It should be auto-discovered when the marketplace is installed.
