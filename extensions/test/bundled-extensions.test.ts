...
const originLine = (line: string) => /https?:\/\/|©|Copyright|All rights reserved|Apache|Modified by example|anthropics\/|trademark|أنثروبيك|NOTICE|origin:/i.test(line)
...
describe("الحزمُ المضمَّنة — عقدُ الاستيراد", () => {
  ...
  for (const id of ids) {
    ...
    test(`${id}: الرخصةُ والإشعارُ وملفُّ الميزات`, () => {
      ...
      expect(notice).toContain("Modified by example")
    })
  }
})
