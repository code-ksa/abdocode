...
const modelSystem = (
  ...
  // الوضعُ الأصيل: التعريفاتُ تحمل الوصفَ والشكل، فالنصُّ فهرسُ أسماءٍ لا نسخةٌ ثانية (وفرٌ مقيس: 17k ⇦ ~0.6k).
  const catalogue = nativeTools
    ? `الأدواتُ المتاحة الآن بأسمائها (شكلُ كلٍّ ووصفُها في تعريفات الأدوات المرفقة بالطلب): ${bindings.map((binding) => binding.exposedName).join("، ")}`
    : bindings.map((binding) => `- ${binding.usage} — ${binding.description}`).join("\n")
...
export function buildChatRequest(input: {
  ...
  const system = modelSystem(prepared.system, input.system, prepared.bindings, input.nativeTools === true)
