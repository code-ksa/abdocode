...
export const TOOLS: readonly ToolSpec[] = [
  ...
  // S8 (09-18) — دفترُ الواجهات: ما دوّنه desk ui وpage عن كلّ شاشةٍ عُمل عليها (الإطار، الوضع، الشجرة، الأنماط، اللقطة، الكودُ المرتبط) — قراءةٌ من الحالة بلا متصفّحٍ ولا سطح مكتب.
  { name: "ui-book", effect: "read", usage: "ui-book [list | show <تطبيق>/<شاشة>]", summary: "دفترُ الواجهات التي عمل عليها الوكيل: لكلّ برنامجٍ أو موقعٍ شاشاتُه المدوَّنة تلقائيّاً عند desk ui وpage (متى، الإطارُ والوضعُ والمقياس، ملخّصُ الشجرة، الأنماطُ المحسوبة، اللقطة، وملفّاتُ الكود المرتبطة في المشروع) — ذاكرةٌ توجّه ولا تغني عن قراءةٍ حيّة", agentCallable: true, runner: "project-read" },
  ...
  { name: "desk", effect: "outside-workspace", usage: "desk open <برنامج|مسار> | desk wait <نصّ> [ث] | desk shot [screen] | desk windows | desk focus <عنوان|pid:رقم> | desk ui [عمق] | desk set <uN> <نص> | desk press <uN> | desk click <x> <y> [right|double] | desk drag <x1> <y1> <x2> <y2> | desk point <وصفُ عنصرٍ في اللقطة> | desk type <نص> | desk key <Enter|ctrl+s|…> | desk scroll <up|down|left|right> [عدد] | desk layout save [<اسم>] | desk layout list | desk layout forget <اسم>", summary: "كومبيوتر-يوس على سطح مكتب المستخدم. الطريقُ الأوّل على أيّ واجهة: desk ui يقرأ شجرةَ الواجهة (الحقولُ والأزرارُ بأسمائها ومراجعها uN) ثمّ desk set/press يملأ ويضغط بالمرجع؛ اللقطةُ (النافذةُ المركَّزة، أو shot screen للشاشة كلِّها) إلى نموذج الرؤية حين لا تعلن الواجهةُ عناصرَها؛ النوافذُ وتركيزُها (عنوان أو pid:)، نقرٌ وكتابةٌ ومفاتيحُ وتمرير. يحتاج «تحكّم سطح المكتب» وموافقةً لكلّ فعل؛ لا إدخالَ بلا desk focus: الإحداثيّاتُ بكسلاتٌ فعليّة من زاوية النافذة، والفعلُ يُلغى إن لم تكن هي المقدّمة", agentCallable: true, runner: "desktop" },
  // المهاراتُ المحلّية كما في كلود: النموذجُ يعرفها من إعلانٍ في النظام ويحمّل ما يناسب المهمّة بنفسه — تعليماتٌ لا صلاحيات.
  { name: "skill", effect: "read", usage: "skill <حزمة/مهارة> | skill list [كلمة] | skill save <اسم> [:: وصف]", summary: "تحميلُ مهارةٍ محلّية مفعَّلة إلى السياق (تعليماتٌ لا صلاحيات، حتى ثلاثٍ في الدور)، أو «save» يقطّر خطواتِ هذا الدور الناجحة (متصفّح/سطح مكتب/سكربت) إلى .abdo/skills/<اسم>/SKILL.md بمتغيّرات (بموافقة) — أو سردُ المتاح بكلمة", agentCallable: true, runner: "framed" },
  ...
  // 09-16 — اعتمادُ الخطّة قبل كود المنتج (كما ExitPlanMode عند كلود): النموذجُ يطلب، والمستخدمُ يعتمد عبر بوّابة الموافقة، والاعتمادُ مربوطٌ ببصمة ABDO-SPRINTS.md فيسقط إن تغيّرت.
  { name: "plan-approve", effect: "edit", usage: "plan-approve", summary: "طلبُ اعتماد خطّة ABDO-SPRINTS.md من المستخدم قبل أيّ كتابةٍ لملفّات المنتج — يمرّ ببوّابة الموافقة، والاعتمادُ يسقط إن تغيّرت الخطّة", agentCallable: true, runner: "framed" },
  ...
  // ب8ج (مقيس 09-14: النموذجُ ردّ «لا أداةَ باسم browser extension» — browser للمشغّل وحده): bridge للنموذج — حالةُ الإضافة، والاقترانُ عند الحاجة.
  { name: "bridge", effect: "outside-workspace", usage: "bridge [status | pair]", summary: "إضافةُ المتصفّح الحقيقيّ: «bridge status» هل الإضافةُ مقترنة ومتّصلة وأيُّ خلفيّةٍ مختارة؛ «bridge pair» يفتح نافذةَ الاقتران ويُقلع المتصفّح إن لم يعمل وينتظر الإضافةَ — وإن غابت يعيد رابطَ تثبيتها لتقوله للمستخدم. بعد الاتّصال تعمل page/open/look/tap/scroll/shot في تبويب المستخدم", agentCallable: true, runner: "surface" },
  ...
  // ن5 (09-16) — التبويبات والتاريخ: مواقعُ النشر تفتح تبويباتٍ وتعود؛ كلُّ انتقالٍ يرفع الجيل ويُبطل المراجع.
  { name: "tabs", effect: "read", usage: "tabs [list | switch <رقم> | close <رقم> | new <رابط>]", summary: "تبويباتُ متصفّح الوكيل (المملوك أو عبر الإضافة): قائمةٌ مرقّمة بالحاليّ، تبديلٌ برقم، إغلاقٌ برقم (بموافقة؛ آخرُ تبويبٍ لا يُغلق)، وفتحُ رابطٍ في تبويبٍ جديد بسياسة المواقع — ثمّ page", agentCallable: true, runner: "surface" },
  { name: "back", effect: "network", usage: "back", summary: "رجوعٌ إلى الصفحة السابقة في تاريخ التبويب (المملوك أو الإضافة) — يرفع الجيل؛ بلا سابقةٍ يُقال", agentCallable: true, runner: "surface" },
  { name: "forward", effect: "network", usage: "forward", summary: "تقدّمٌ إلى الصفحة اللاحقة في تاريخ التبويب — يرفع الجيل؛ بلا لاحقةٍ يُقال", agentCallable: true, runner: "surface" },
  // S6 (09-18) — الجلساتُ والتاريخ: المتصفّحُ المملوك يبقى داخلاً بعد إعادة التشغيل (ملفٌّ دائم + حالةُ تخزينٍ لكلّ أصل)، وتاريخُ آخر ٢٠٠ زيارة يُعاد فتحُه برقم.
  { name: "sessions", effect: "read", usage: "sessions [list | save <أصل> | forget <أصل> | restore <أصل>]", summary: "جلساتُ الدخول المحفوظة للمتصفّح المملوك لكلّ أصل (الأصلُ والحالةُ وآخرُ دخولٍ والعمر): save يصدّر كعكاتِ الأصل ومخزنَه إلى قرص المستخدم بعد دخولٍ نجح، restore يستعيدها ثمّ يعيد التحميل (بموافقة التنقّل)، forget يحذف ملفَّها ويسمّيه — لا كلمةَ سرٍّ تُخزَّن أبداً", agentCallable: true, runner: "surface" },
  { name: "history", effect: "read", usage: "history [list | open <رقم>]", summary: "تاريخُ متصفّح الوكيل: آخرُ ٢٠٠ زيارة مرقّمةً الأحدثُ أوّلاً (العنوانُ والرابطُ والعمر)، وopen <رقم> يعيد فتحَ السطر بسياسة المواقع والبوّابة نفسِها", agentCallable: true, runner: "surface" },
  ...
  // S9 (09-18) — تصميمُ الواجهات بلا Canva: مواصفةٌ آليّة من الصفحة الهدف (DOM) أو من لقطتها (نموذجُ الرؤية)، رندرٌ حتميّ منها، وقياسُ القرب بالبكسل.
  { name: "design", effect: "read", usage: "design spec <page | rN | رابط | ملف.png | ملف.json> [ملف الإخراج.json] | design render <spec.json> <out.html> | design compare <target.png> <candidate.png> | design shot <اسم>", summary: "تصميمُ واجهةٍ على مثال هدف بلا كانفا: «spec» يستخرج من الصفحة الموصولة أو رابطٍ أو عنصرٍ بمرجعه أو لقطةٍ مواصفةً آليّة (المنفذ، الألوان بالاستعمال، الخطوط بالأدوار، الأقسام بصناديقها، المكوّنات، التباعد، الأصول) وتُحفظ JSON؛ «render» يولّد HTML حتميّاً منها (RTL حين تكون rtl)؛ «shot» يحفظ لقطةَ الصفحة؛ «compare» يعطي نسبةَ التطابق بالبكسل وأكثرَ المناطق اختلافاً — كرّر spec⇦render⇦compare حتى يطابق.", agentCallable: true, runner: "design" },
  ...
  // ن4 — انتظارٌ محدود لظهور نصٍّ أو مرجعٍ في الصفحة بعد فعل (بدل قراءةٍ مبكّرة فتكرار).
  { name: "wait", effect: "read", usage: "wait <نصّ أو [rN]> [ثوانٍ]", summary: "ينتظر ظهورَ نصٍّ أو مرجعٍ في الصفحة (المملوكة أو عبر الإضافة) حتى ١٥ ث افتراضاً (≤ ٦٠)، ويسمّي السطرَ الذي ظهر أو يقول إنّه لم يظهر مع أوّل سطور الصفحة — بعد open/tap/fill/key قبل page", agentCallable: true, runner: "surface" },
