...
export class ModelRequestFailure extends Error {
  ...
  publicMessage(language: string | undefined): string {
    ...
    const details = this.failure.kind === "credential"
      ...
      // مقيس 09-17: «تعذّر الوصول — تحقّق من الاتصال» أوحى للمالك أنّ العيبَ عنده، والواقعُ 503 «Service temporarily overloaded» من المزوّد.
      : this.failure.kind === "provider-unavailable"
      ? ar ? "المزوّد مزدحم أو متوقّف مؤقّتاً — ليست مشكلة اتصالك؛ أُعيدت المحاولة تلقائياً عدة مرات. اكتب «اكمل» لاستئناف ما توقّف حين يعود، أو اضبط سلّم التصعيد في الإعدادات ← المزوّدون ليتحوّل تلقائياً إلى نموذج آخر." : "The provider is overloaded or temporarily down — not your connection; the request was retried automatically several times. Type «اكمل» to resume once it is back, or set a model ladder in Settings → Providers so the agent switches to another model automatically."
