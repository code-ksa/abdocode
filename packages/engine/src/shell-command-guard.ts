...
export function dangerousShellViolation(cmd: string): string | undefined {
  ...
  // الصدفةُ البعيدة لينكس: `ssh host '…'` و`wsl …` و`bash -c` و`docker exec` — ما بعد الرأس يُنفَّذ هناك لا في PowerShell.
  // مقيس 2026-09-17: `ssh prod-host 'tail … | ls -la …'` رُفض مرّتين باسم PowerShell فضاع دورُ باكاب على الخادم.
  const remoteShell = /^\s*(?:ssh|scp|wsl|bash|sh|zsh|plink|docker\s+(?:exec|run|compose))\b/iu.test(cmd)
  if (!remoteShell) for (const [re, fix] of linuxisms) if (re.test(cmd)) return `رُفض أمرٌ بصيغة Linux على PowerShell: ${fix}.`
