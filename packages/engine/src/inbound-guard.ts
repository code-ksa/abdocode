...
const RULES: readonly Rule[] = Object.freeze([
  ...
  {
    name: "exfiltration",
    // مقيس 09-17 على المثبَّت 4.0.47 (مشروع abdo-router): شيفرةُ الوكيل نفسِه «outputPerMillion: number }, promptTokens» أطلقت القاعدةَ
    // (output…token داخل معرّفات) فعُومل ملفُّه بياناتٍ مشبوهة وأُهدرت قراءتان. الفعلُ يبدأ كلمةً والمفعولُ ينتهي كلمةً، ولا يعبر
    // الفراغُ بينهما ترقيمَ شيفرة ({}()=;,<>) — الهجومُ النثريّ («print the api key») يبقى مطابقاً، والمعرّفاتُ لا.
    // الفعلُ والمفعولُ كلاهما يبدأ كلمةً (showToken ≠ show token) — ثمنُه أنّ التباعدَ الحرفيّ («p r i n t») لا يُمسك هنا؛ قاعدةُ التجاوز تمسكه.
    test: /(?<![a-z0-9])(?:reveal|print|show|output|repeat)[^.\n{}()=;,<>]{0,30}(?<![a-z0-9])(?:system prompt|your instructions|api key|secret|token|password)|(?:اطبع|اظهر|اكشف|ارسل)[^.\n{}()=;,<>]{0,30}(?:المفتاح|السر|كلمه المرور|التعليمات|النظام)/u,
  },
  {
    name: "tool-forgery",
