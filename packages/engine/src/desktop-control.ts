...
/**
 * ب1 — عقدُ سطح المكتب عبر الأنظمة (خطُّ أتمتة الأنظمة، أمرُ المالك 09-14): الأداةُ `desk` تتكلّم مع **قناة** لا مع ويندوز.
...
export const windowsBackend: DesktopBackend = Object.freeze({
  ...
  run: (action, options) => runDesktop(action, options),
})
/** القناةُ لهذا النظام — أو رفضٌ مسمّى (لا سقوطَ صامت إلى ويندوز على غيره). */
export const desktopBackendFor = (platform: NodeJS.Platform = process.platform): DesktopBackend | { readonly error: string } =>
  platform === "win32" ? windowsBackend : { error: `لا قناةَ سطح مكتب لهذا النظام بعد (${platform}) — الخطُّ (ب): linux-x11 عبر xdotool + AT-SPI، ثمّ wayland وmacos.` }
