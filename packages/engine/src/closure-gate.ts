 ...
 */
/** نصُّ الهدف بلا أوامر سطح المكتب (desk …) وبلا ذكرٍ منفيّ للمتصفّح («لا/بدون/دون تفتح المتصفّح») — هذان لا يطلبان دليلَ متصفّح. */
export const browserGoalText = (goal: string): string =>
  goal
    .replace(/(?<![\p{L}\p{N}_-])desk\s+[a-z]+/giu, " ")
    .replace(/(?:^|[^\p{L}])(?:لا|ولا|بدون|دون|بلا|ليس|غير|never|without|don't|do not)\s+(?:\S+\s+){0,2}?(?:المتصفّح|المتصفح|متصفّح|متصفح|browser)/giu, " ")
