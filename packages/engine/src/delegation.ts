 ...
 *
 * والعمقُ مسقوف: الأب يفوّض، والطفلُ **القارئ** يوازي قرّاءً قلائل (09-16)،
 * والحفيد لا يفوّض. تفويضٌ متداخلٌ بلا حدّ شجرةٌ تنفق سقفَ الدور في فرعٍ لا
 * يراه أحد؛ وقارئٌ يستدعي كاتباً تصعيدٌ فوق سقفه — كلاهما مرفوضٌ بالاسم.
 *
...
/** أداةُ الفريق المتوازي (هـ2) — للطفل القارئ وحده إن أعلنها، وتحت سقف العمق. */
export const TEAM_TOOL = "team"

/** لوحُ الخطّة — أداةُ الدور الأب وحده. */
export const PLAN_TOOL = "plan"

/** عمقُ التداخل الأقصى: الأب يفوّض (0⇦1)، والطفلُ القارئ يوازي قرّاءً (1⇦2)، والحفيد لا. */
export const MAX_DELEGATION_DEPTH = 2

/** سقوفُ الطفل — أضيق من سقوف الدور بالبناء؛ رُفعت 09-16 (3 حِقب/6 جولات/1200 حرف كانت تقطع أيّ بحثٍ عميق قبل تمامه). */
export const CHILD_MAX_EPOCHS = 10
export const CHILD_MAX_ROUNDS = 15
export const REPORT_MAX_CHARS = 4000
export const REPORT_MAX_COMMANDS = 12
export const REPORT_MAX_REFUSALS = 6
/** سقفُ فريق الطفل القارئ — قرّاءٌ متوازون قلائل، لا شجرة. */
export const NESTED_TEAM_MAX = 3
...
export const childToolRefusal = (agent: AgentDefinition, word: string, depth: number): string | undefined => {
  ...
  // التفويضُ الفرديّ ملكُ الدور: طفلٌ يفوّض طفلاً يضاعف الأثر بلا توازٍ يبرّره.
  if (trimmed === DELEGATE_TOOL) {
    return `رُفضت «${trimmed}»: الوكيل المفوَّض لا يفوّض فردياً (عمق ${depth}) — القارئُ يوازي بـ«${TEAM_TOOL}» لقرّاءٍ فقط.`
  }
  if (trimmed === TEAM_TOOL) {
    if (depth >= MAX_DELEGATION_DEPTH) return `رُفضت «${trimmed}»: بلغ سقفَ العمق ${MAX_DELEGATION_DEPTH} (عمق ${depth}) — الحفيد لا يفوّض.`
    if (!agent.readOnly) return `رُفضت «${trimmed}»: فريقُ الطفل للوكيل القارئ وحده — «${agent.name}» يكتب/ينفّذ فلا يوازي.`
    // قارئٌ تحت السقف: يبقى شرطُ الإعلان في سقفه (الغياب رفضٌ لا إذن) — يُفحص أدناه.
  }
...
export const childBrief = (agent: AgentDefinition, task: string): string =>
  ...
  (agent.readOnly && agent.callable.has(TEAM_TOOL)
    ? `لا تفوّض فردياً؛ إن اتّسع البحث فوازِ بـ«${TEAM_TOOL}» حتى ${NESTED_TEAM_MAX} وكلاءِ قراءةٍ فقط. أنهِ بجوابٍ واحدٍ موجز يصلح تقريراً لمن فوّضك.\n`
