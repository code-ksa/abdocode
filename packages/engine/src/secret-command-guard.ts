...
/**
 * مسارُ مستودعٍ طويل ليس سرّاً (مقيس 2026-09-17: مسارُ سكربتِ قياسٍ من ٤١ محرفاً بثلاث شرطاتٍ مائلة حُجب في إيصال
 * أداةٍ فضاع الدليلُ على المحكّم). المسارُ مقاطعُ أحرفٍ صغيرة/أرقام/شرطات تفصلها شرطاتٌ مائلة، بلا حرفٍ كبير ولا `+`/`=`؛
 * والسرُّ المُرمَّز base64 الذي يحمل `/` يخلط الحالتين أو يحمل `+`/`=` فيبقى محجوباً (توأمٌ إيجابيّ في الاختبار).
 */
const REPO_PATH_SHAPE = /^[a-z0-9._-]+(?:\/[a-z0-9._-]+)+$/u
const isRepoPath = (value: string): boolean => REPO_PATH_SHAPE.test(value)

/** ما بقي شبيهاً بسرٍّ بعد الحجب — بصمات الدليل (sha256) ومسارات المستودع ليست منه. */
export function residualSecretMatches(text: string): string[] {
  if (typeof text !== "string" || text.length === 0) return []
  const found: string[] = []
  for (const match of text.matchAll(SECRETISH_RESIDUAL_G)) {
    if (EVIDENCE_DIGEST.test(match[0]) || isRepoPath(match[0])) continue
    found.push(match[0])
  }
  return found
}

/** مصفاةٌ أخيرة قبل الكتابة: كلُّ بقيّةٍ طويلة تُحجب (بصمة الدليل ومسار المستودع يبقيان). */
export function sweepResidualSecrets(text: string): { text: string; redactions: number } {
  ...
  const out = text.replace(SECRETISH_RESIDUAL_G, (match: string) => {
    if (EVIDENCE_DIGEST.test(match) || isRepoPath(match)) return match
    redactions += 1
    return REDACTED
  })
  return { text: out, redactions }
