...
describe("project map — S1 cold start", () => {
  ...
  test("09-16: with a goal, the map adds the nearest files from the project index — symbol before text — and nothing when nothing matches (twin)", async () => {
    const dir = mkdtempSync(join(tmpdir(), "abdo-map-goal-"))
    mkdirSync(join(dir, "src"), { recursive: true })
    mkdirSync(join(dir, "node_modules", "dep"), { recursive: true })
    writeFileSync(join(dir, "package.json"), "{}")
    writeFileSync(join(dir, "src", "invoice-service.ts"), "export class InvoiceService { total() { return 1 } }")
    writeFileSync(join(dir, "src", "notes.md"), "ملاحظات عن الفوترة والفواتير")
    writeFileSync(join(dir, "src", "unrelated.ts"), "export const x = 1")
    writeFileSync(join(dir, "node_modules", "dep", "index.ts"), "export class InvoiceService {}")
    const map = await projectMap(dir, 80, "أصلح InvoiceService في الفوترة")
    expect(map).toContain("الأقرب للهدف")
    expect(map).toContain("src/invoice-service.ts (class InvoiceService) [symbol]")
    expect(map).toContain("src/notes.md [fts]") // العربيّةُ تُسترجع نصّاً (كانت التجزئة لاتينيّةً فقط)
    expect(map).not.toContain("unrelated.ts [")
    expect(map).not.toContain("node_modules/dep")
    expect(await projectMap(dir, 80, "zzz-nothing-here")).not.toContain("الأقرب للهدف")
    expect(await projectMap(dir)).not.toContain("الأقرب للهدف")
  })
})
