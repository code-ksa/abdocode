...
describe("S13.5 — الأداة في السجلّ الواحد", () => {
  ...
  test("سقفُ العمق يُقاس في القشرة، ويُنقص في finally مهما جرى", () => {
    ...
    expect(MAX_DELEGATION_DEPTH).toBe(2) // 09-16: الطفلُ القارئ يوازي قرّاءً؛ الحفيد لا
    // فريقُ الطفل: قارئٌ فقط، وقرّاءٌ فقط، وبسقفٍ أضيق — مسامير على الحارس في القشرة.
    expect(source).toContain("if (nested && (caller === undefined || !caller.readOnly)) return denied(")
    expect(source).toContain("if (nested && !agent.readOnly) return denied(")
    expect(source).toContain("const teamCap = nested ? Math.min(work.parallelAgents, NESTED_TEAM_MAX) : work.parallelAgents")
  })
...
describe("S13.5 — المفتاحان الميتان صار لهما قارئ", () => {
  ...
  test("القارئان حقيقيّان: التوزيع والإعلان يسألان delegation، والكتالوج يسأل reviewer", () => {
    expect(source).toContain('if (!delegationEnabled()) {')
    // 4.0.52: «أقصى» بلا فريقٍ كان يَعِد ولا يُنفّذ — القارئُ صار دالّةً: المفتاح أو وضعُ عملٍ يوازي.
    expect(source).toContain('const delegationEnabled = (): boolean => pluginOnNow("delegation") ||')
    expect(source).toContain('const delegationOn = delegationEnabled()')
    expect(source).toContain('const reviewerOn = pluginOnNow("reviewer")')
    // هـ2 (2026-09-07) كان أربعةَ قرّاء مباشرين؛ و4.0.52 جمعهم في `delegationEnabled()` فصار المفتاحُ يُقرأ
    // مرّةً واحدة داخلها، والقرّاءُ (الإعلان، delegate، team، الوكيلُ الموجِّه) يسألونها هي.
    expect(source.match(/pluginOnNow\("delegation"\)/gu)).toHaveLength(1)
    expect(source.match(/delegationEnabled\(\)/gu)?.length ?? 0).toBeGreaterThanOrEqual(4)
...
describe("S13.5 — الإعلان يساوي الإذن", () => {
  test("سقفُ الوكيل يقصّ كتالوج النموذج (الثغرة «أ» مغلقة)", () => {
    ...
    // 09-16: team تُعلَن للطفل إن أعلنها في سقفه (القارئُ يوازي قرّاءً)؛ delegate لا تُعلَن لطفلٍ أبداً.
    expect(source).toContain(".filter((tool) => withinAllowlist(tool.name) && (tool.name !== DELEGATE_TOOL || (delegationOn && allowlist === undefined)) && (tool.name !== TEAM_TOOL || (delegationOn && (allowlist === undefined || allowlist.includes(TEAM_TOOL)) && workProfile(loadSettings().workMode).parallelAgents >= 2)))")
