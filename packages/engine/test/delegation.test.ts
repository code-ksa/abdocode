...
/** عدسةٌ ضيّقة لا تعلن team — التوأمُ السلبيّ للقرّاء المعلِنين (09-16). */
const recaller = findAgent(CATALOGUE, "recaller")!
...
describe("delegation — سقفُ الوكيل بالبناء", () => {
  ...
  test("depth cap: a child may not delegate singly — refused by name, at depth 1", () => {
  ...
  test("09-16: a read-only child that declares team may team at depth 1; a writer may not; nobody at depth 2; undeclared is refused", () => {
    const readerWithTeam: AgentDefinition = { ...reviewer, tools: [...reviewer.tools, TEAM_TOOL], callable: new Set([...reviewer.callable, TEAM_TOOL]) }
    expect(readerWithTeam.readOnly).toBe(true)
    expect(childToolRefusal(readerWithTeam, TEAM_TOOL, 1)).toBeUndefined()
    expect(childToolRefusal(readerWithTeam, TEAM_TOOL, 2)).toContain("سقفَ العمق")
    expect(childToolRefusal(recaller, TEAM_TOOL, 1)).toContain("لا يعلنها") // الغياب رفضٌ لا إذن
    // القرّاءُ المشحونون الثلاثة يعلنونها ويبقون قراءةً-فقط (أثرُ team قراءة)
    for (const name of ["orient", "planner", "reviewer"]) {
      const agent = findAgent(CATALOGUE, name)!
      expect(agent.readOnly).toBe(true)
      expect(childToolRefusal(agent, TEAM_TOOL, 1)).toBeUndefined()
    }
    const writerWithTeam: AgentDefinition = { ...builder, tools: [...builder.tools, TEAM_TOOL], callable: new Set([...builder.callable, TEAM_TOOL]) }
    expect(childToolRefusal(writerWithTeam, TEAM_TOOL, 1)).toContain("القارئ وحده")
    expect(childToolRefusal(readerWithTeam, DELEGATE_TOOL, 1)).toContain("لا يفوّض")
  })

  test("09-16: the brief tells a team-capable reader it may parallelise readers, and everyone else not to delegate", async () => {
    const readerWithTeam: AgentDefinition = { ...reviewer, tools: [...reviewer.tools, TEAM_TOOL], callable: new Set([...reviewer.callable, TEAM_TOOL]) }
    const h = harness()
    await h.run(readerWithTeam, ["تم"])
    expect(h.prompts[0]).toContain(`«${TEAM_TOOL}»`)
    expect(h.prompts[0]).toContain("وكلاءِ قراءةٍ فقط")
    expect(h.prompts[0]).not.toContain("لا تفوّض إلى وكيلٍ آخر")
  })
  ...
  test("the advertised ceiling handed to the model equals the declared one", async () => {
    ...
    expect(h.prompts[0]).toContain("وكلاءِ قراءةٍ فقط") // reviewer يعلن team (09-16)
    const narrow = harness()
    await narrow.run(recaller, ["تم"])
    expect(narrow.prompts[0]).toContain("لا تفوّض إلى وكيلٍ آخر")
  })
})
...
describe("delegation — المدخل والتقرير", () => {
  ...
  test("the child's own epoch counter and awareness are its own", async () => {
    ...
    expect(report.epochs).toBe(2)
    expect(CHILD_MAX_EPOCHS).toBe(10) // 09-16: ثلاثٌ كانت تقطع البحث العميق قبل تمامه
  })
})
