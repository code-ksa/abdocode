...
describe("token economy wiring in the live cli path", () => {
  test("both ledger writers forward decoded usage through chargeableUsage behind plugins.cacheAccounting", () => {
    ...
    // Fifth writer (2026-09-16, ن7): look-and-point (visionPointAsk) spends one bounded vision call on the
    // selected provider, tagged note:"vision.point" so the ledger can split it. Raised honestly 4 -> 5.
