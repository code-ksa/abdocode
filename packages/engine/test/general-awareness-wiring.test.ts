...
describe("S13.3 — المفتاح يمرّ بسجلّ المكوّنات وحده", () => {
  ...
  test("القراءة من الجرد لا من الإعدادات مباشرة، ومرةً واحدة لكل دور", () => {
    ...
    expect(source.match(/pluginOnNow\("generalAwareness"\)/gu)).toHaveLength(3) // ت1 (09-14): القارئُ الثالث دروسُ الإصدار عند إقلاع serve
  })
})
...
describe("S13.3 — المخزن في دليل التثبيت، والكتابة خلف المفتاح", () => {
  ...
  test("الطفرة (ج): الكتابة داخل شرط المفتاح، وكتابةٌ واحدة لا غير", () => {
    ...
    // ت1 (09-14): الكاتبُ الثاني دروسُ الإصدار — خلف مفتاح الوعي العامّ نفسِه، داخل applyShippedReleaseLessons وحدها.
    expect(source.match(/writeFileSync\(GENERAL_STORE_PATH/gu)).toHaveLength(2)
    const lessonsFn = source.indexOf("const applyShippedReleaseLessons = (): void => {")
    const lessonsGate = source.indexOf('if (!pluginOnNow("generalAwareness")) { process.stderr.write("📚 دروسُ الإصدار: الوعي العامّ معطَّل', lessonsFn)
    const lessonsWrite = source.indexOf("writeStore: (text) => writeFileSync(GENERAL_STORE_PATH, text, \"utf8\")", lessonsGate)
    expect(lessonsFn).toBeGreaterThan(0)
    expect(lessonsGate).toBeGreaterThan(lessonsFn)
    expect(lessonsWrite).toBeGreaterThan(lessonsGate)
