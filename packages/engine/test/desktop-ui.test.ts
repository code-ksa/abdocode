...
describe("السكربت — الوعيُ بالـDPI وUTF-8 وتحديدُ pid والشجرة", () => {
  ...
  test("حارسُ المقدّمة يرفع النافذةَ المربوطة حين يكون السارقُ تطبيقَنا نحن، ويرفض غيرَه — مقيس: «focus moved (AbdoCode)» بين نداءين", () => {
    ...
    expect(ours).toContain("if ($selfPids -contains [int]$fpid) { [void][AbdoDesk]::Focus($want)")
  ...
  test("ب1 — عقدُ القناة: ويندوز هو القناةُ الوحيدة اليوم، وغيرُه رفضٌ مسمّى لا سقوطٌ صامت", async () => {
    ...
    expect("error" in linux && linux.error).toContain("linux-x11")
