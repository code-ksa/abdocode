...
describe("browser proof (2026-09-13): a goal asking to open/shoot/click is not closed by a green build", () => {
  ...
  // مقيس 09-16 على المثبَّت 4.0.40: مهمّةُ سطح مكتبٍ («desk shot»، «لا تفتح المتصفّح») أشعلت الشرطَ فطالب بـshot full ودفع النموذجَ نحو المتصفّح.
  test("desktop tool words and a negated browser mention never ask for browser proof; a real browser ask still does (twin)", () => {
    const deskGoal = "نفّذ: desk open mspaint ثم desk drag 300 300 520 420 ثم desk shot — ولا تفتح المتصفّح"
    expect(browserProofVerdict(deskGoal, [{ command: "desk shot", output: "لقطةُ نافذة", verdict: okv }])).toBeUndefined()
    expect(browserProofVerdict("افعل ذلك بدون المتصفّح والتقط لقطةَ الشاشة بـdesk shot", [])).toBeUndefined()
    expect(browserProofVerdict("do it without the browser and take a desk shot", [])).toBeUndefined()
    expect(browserProofVerdict("افتح الصفحة في متصفّحك والتقط لقطة (shot full)", [])).toContain("shot full")
  })
})
