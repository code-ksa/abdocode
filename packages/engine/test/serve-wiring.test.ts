...
const composerSource = await Bun.file(new URL("../src/prompt-composer.ts", import.meta.url)).text()
const legacyLines = await Bun.file(new URL("../src/prompt-legacy-lines.ts", import.meta.url)).text()
...
describe("serve convergence wiring", () => {
  ...
  test("the assistant-identity leak guard still matches the shipped product name", async () => {
    ...
    expect(legacyLines).toContain(`لا تضع «${guard}» في ملفات المشروع`)
  ...
  test("every declared toggle is read through the registry — or honestly marked unread — and nothing bypasses the resolver", async () => {
    ...
    for (const d of PLUGINS) {
      ...
      if (d.name === "usageMeter") {
        // ذ5: قارئٌ واحد عند النداء داخل meterCall — والمواضعُ الستّة (البوّابة، الاستنتاج، المرتِّب،
        // النداءُ المفكوك، البثُّ المحلّيّ، ونداءُ الرؤية الجانبيّ ن7 09-16) تنادي الدالّة لا المفتاح.
        expect(d.site).toBe("call")
        expect(source.match(/pluginOnNow\("usageMeter"\)/gu)).toHaveLength(1)
        expect(source.match(/meterCall\(prov/gu)).toHaveLength(6)
        continue
      }
      ...
      if (d.name === "delegation" || d.name === "reviewer") {
        ...
        // 09-17: القرّاءُ الثلاثة يمرّون من delegationEnabled() (المفتاحُ أو وضعُ عملٍ يوازي) — قارئٌ واحد للمفتاح وأربعةُ مواضع للدالّة.
  ...
  test("the intent field is wired behind plugins.intentField, defaults OFF, and OFF constructs nothing", async () => {
    ...
    // S1 (09-17): المفتاحُ يصل المُركِّبَ مدخلاً واحداً (`intentField`) فيحكم المسارَين الأصيلَ والنصّيَّ هناك.
    expect(source).toContain("intentField: hooks.intentField === true,")
    expect(composerSource).toContain("nativeAgentSystem(input.planningPhase, input.sprintPlan.required, input.intentField, input.policy)")
    expect(composerSource).toContain('(input.intentField ? intentInstruction() : "") +')
