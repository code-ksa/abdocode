...
// المهلةُ المسطّحة: العقدُ المفحوص هو **عددُ** المحاولات (سبع) لا زمنُها — بلا هذا تتجاوز السبعُ مهلةَ الاختبار.
process.env.ABDO_MODEL_RETRY_BACKOFF_MS = "20"
...
test.skipIf(process.platform !== "win32")("سبعُ 503 متتالية: السقفُ يعضّ — سبعةُ طلباتٍ بالضبط ثمّ رفضٌ يسمّي HTTP 503", async () => {
  // 4.0.46 رفع السقفَ من خمسٍ إلى سبعٍ بقياس: خمسُ محاولاتٍ في ٤٠ ثانية لم تكفِ ازدحامَ NIM. العددُ هنا هو العقد.
  const r = await run([503, 503, 503, 503, 503, 503, 503])
  expect(r.requests).toBe(7)
  expect(r.end.kind).toBe("refused")
  expect(r.end.why).toContain("(HTTP 503)")
}, 240_000)

test.skipIf(process.platform !== "win32")("خمسُ 503 ثمّ 200: لا استسلامَ قبل السقف — ستّةُ طلباتٍ ودورٌ مكتمل (التوأمُ الإيجابيّ)", async () => {
