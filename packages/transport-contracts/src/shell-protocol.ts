...
export const SHELL_FRAMES: readonly FrameSpec[] = [
  ...
  // خوادمُ التطوير في لوحة المتصفّح (S2): إعداداتُ الإطلاق من `.claude/launch.json` تُعرض أزراراً في
  // الحالة الفارغة؛ التشغيلُ والإيقافُ بيد المشغّل، والحالةُ تُقاس لحظةَ الطلب كلوح الخوادم.
  inbound("dev-servers", "طلبُ صفوف خوادم التطوير: إعداداتُ الإطلاق بحالتها المقيسة الآن", [], [], true),
  inbound("dev-server-start", "تشغيلُ إعداد إطلاقٍ باسمه تحت إدارة المحرّك في مجلد المشروع", ["name"], ["name"]),
  inbound("dev-server-stop", "إيقافُ خادم تطويرٍ مُدارٍ بمنفذه — عند إغلاق تبويبه أو بزرّه", ["port"], ["port"]),
  // تاريخُ متصفّح الوكيل (S6): اللوحةُ تطلب آخرَ الزيارات فيقرأ المحرّكُ ملفَّه لحظتَها ويردّ؛ الفتحُ في اللوحة بيد المشغّل.
  inbound("browser-history", "طلبُ تاريخ متصفّح الوكيل: آخرُ الزيارات الأحدثُ أوّلاً", [], [], true),
  ...
  // ن9 (09-16) — بابُ سياسة المواقع: المحرّكُ يطلب («browser allow|block <نطاق>») والقشرةُ تحفظ بالطريق الأصيل نفسه (workspace_store_set) — لا كتابةَ من المحرّك في workspace-v1.json.
  { kind: "site-policy", dir: "out", required: ["op", "site"], summary: "طلبُ تعديل سياسة المواقع المحفوظة من الشات: op=allow|block وsite نطاقٌ — تحفظه القشرة بتحقّقها وتُعلن" },
  ...
  { kind: "dev-servers", dir: "out", required: ["rows"], summary: "خوادمُ التطوير من إعدادات الإطلاق: اسمٌ ومنفذٌ ورابطٌ وحالةٌ مقيسة (measuring/up/down) ومشكلاتُ الملفّ إن وُجدت" },
...
export const validateShellFrame = (value: unknown): ShellValidation => {
  ...
  } else if (kind === "server-stop" || kind === "dev-server-stop") {
    if (!Number.isSafeInteger(value.port) || (value.port as number) < 1 || (value.port as number) > 65_535) return invalidField(kind, "port")
  } else if (kind === "recall") {
