...
describe("R9 transport contracts", () => {
  test("owns and validates every current product shell frame", () => {
    ...
    expect(validateShellFrame({ kind: "dev-server-start", name: "web app" }).ok).toBe(false)
    expect(validateShellFrame({ kind: "dev-server-start" }).ok).toBe(false)
    expect(validateShellFrame({ kind: "dev-server-stop", port: 3210 })).toEqual({ ok: true })
    expect(validateShellFrame({ kind: "dev-server-stop", port: "3210" }).ok).toBe(false)
    expect(validateShellFrame({ kind: "server-stop", port: 0 }).ok).toBe(false)
