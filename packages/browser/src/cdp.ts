...
import { SPEC_WALKER_LIMIT, specWalkerExpression } from "./spec-walker"
...
/** ن5 — تبويبٌ (هدفٌ من نوع page) على المنفذ، والحاليُّ هو ما تقوده القناة. */
...
export type CdpHistoryResult = { readonly ok: true; readonly url: string; readonly title: string } | { readonly ok: false; readonly why: string }

/** S6 — كعكةٌ بحقولها الكاملة كما يعيدها Network.getCookies ويقبلها Network.setCookies. قيمتُها للقرص لا للإيصالات. */
...
export class CdpBrowser {
  ...
  /** يعثر على صفحةٍ قابلةٍ للقيادة على المنفذ، أو يفشل باسمه. `targetId` (ن5) يختار تبويباً بعينه عند التبديل. */
  async attach(match?: RegExp, targetId?: string): Promise<void> {
    const res = await fetch(`http://127.0.0.1:${this.port}/json`, { signal: AbortSignal.timeout(4000) })
    const targets = (await res.json()) as CdpTarget[]
    const page = targets.find((t) => t.type === "page" && (targetId !== undefined ? t.id === targetId : match === undefined || match.test(t.url)))
    if (page === undefined) throw new Error(targetId !== undefined ? `لا تبويبَ بالمعرّف ${targetId} على المنفذ ${this.port}` : `لا صفحةً قابلةً للقيادة على المنفذ ${this.port}`)
...
${TREE_HELPERS}
  ...
  // S7 (09-18) — عرضُ المنفذ ومقياسُ الجهاز (devicePixelRatio) مع الارتفاع: خريطةُ المنفذ تحتاجهما لتقول في أيّ إطارٍ ومقياسٍ أُخذت اللقطة.
  async pageMetrics(): Promise<{ readonly viewportWidth: number; readonly viewportHeight: number; readonly scrollHeight: number; readonly scrollY: number; readonly dpr: number }> {
    await this.#guardCurrentPage()
    const raw = await this.#eval("JSON.stringify({ viewportWidth: innerWidth, viewportHeight: innerHeight, scrollHeight: Math.max(document.documentElement.scrollHeight, document.body ? document.body.scrollHeight : 0), scrollY: Math.round(scrollY), dpr: devicePixelRatio })")
    try { const m = JSON.parse(raw) as { viewportWidth: number; viewportHeight: number; scrollHeight: number; scrollY: number; dpr: number }; return { viewportWidth: Math.max(1, m.viewportWidth | 0), viewportHeight: Math.max(1, m.viewportHeight | 0), scrollHeight: Math.max(1, m.scrollHeight | 0), scrollY: Math.max(0, m.scrollY | 0), dpr: typeof m.dpr === "number" && m.dpr > 0 ? Math.round(m.dpr * 1000) / 1000 : 1 } } catch { return { viewportWidth: 1280, viewportHeight: 900, scrollHeight: 900, scrollY: 0, dpr: 1 } }
  }
   ...
   * الوكيلُ نسخ بنيةَ منصّةُ المواقع بألوانه الافتراضيّة لأنّ الشجرةَ النصّيّة لا تحمل لوناً — هذه تحمله بالأرقام.
   */
  ...
  /**
   * S9 (2026-09-18) — مواصفةُ التصميم: شجرةُ العناصر الظاهرة المسطّحة بصناديقها وأنماطها المحسوبة (الماشي الواحد
   * `SPEC_WALKER` — النصُّ نفسُه الذي تحقنه إضافةُ المتصفّح). بمرجعٍ يُمشى من ذلك العنصر، وبدونه من body. قراءةٌ بلا أثر.
   */
  async readSpec(rootRef = "", limit = SPEC_WALKER_LIMIT): Promise<string> {
    await this.#guardCurrentPage()
    return this.#eval(specWalkerExpression(limit, rootRef))
  }
  ...
  /** ن5 — التبويبات المفتوحة على المنفذ (صفحاتٌ فقط) من /json لا من ذاكرة، والحاليُّ يُعلَّم. */
  async listTabs(): Promise<readonly CdpTabInfo[]> {
    const res = await fetch(`http://127.0.0.1:${this.port}/json`, { signal: AbortSignal.timeout(4000) })
    const targets = (await res.json()) as (CdpTarget & { title?: unknown })[]
    return targets.filter((t) => t.type === "page").map((t) => ({ id: t.id, title: String(t.title ?? "").slice(0, 120), url: t.url, current: t.id === this.#pageTarget }))
  }

  /** ن5 — الانتقالُ إلى تبويبٍ بمعرّفه: يُنشَّط في المتصفّح ثمّ تُعاد قناةُ القيادة إليه فتُثبَّت سياسةُ التنقّل من جديد؛ حلقتا الطرفيّة والشبكة تُفرَغان. */
  async switchTab(targetId: string): Promise<CdpTabInfo> {
    const tab = (await this.listTabs()).find((t) => t.id === targetId)
    if (tab === undefined) throw new Error(`لا تبويبَ بالمعرّف ${targetId}`)
    if (tab.url !== "about:blank" && !this.#allowed(tab.url)) throw new Error("Page access blocked by saved site permissions")
    await this.#send("Target.activateTarget", { targetId }, null).catch(() => undefined)
    this.close()
    this.#console = []
    this.#network.clear()
    await this.attach(undefined, targetId)
    return { ...tab, current: true }
  }

  /** ن5 — إغلاقُ تبويب؛ آخرُ تبويبٍ لا يُغلق (يُفصل السطح بدلاً). إغلاقُ الحاليّ ينقل القيادةَ إلى أوّل الباقين. */
  async closeTab(targetId: string): Promise<{ readonly closed: CdpTabInfo; readonly now?: CdpTabInfo }> {
    const tabs = await this.listTabs()
    const tab = tabs.find((t) => t.id === targetId)
    if (tab === undefined) throw new Error(`لا تبويبَ بالمعرّف ${targetId}`)
    if (tabs.length <= 1) throw new Error("آخرُ تبويبٍ لا يُغلق — استعمل surface off لفصل السطح")
    const wasCurrent = targetId === this.#pageTarget
    await this.#send("Target.closeTarget", { targetId }, null)
    if (!wasCurrent) return { closed: tab }
    await Bun.sleep(300)
    const remaining = (await this.listTabs()).find((t) => t.id !== targetId) ?? tabs.find((t) => t.id !== targetId)!
    return { closed: tab, now: await this.switchTab(remaining.id) }
  }

  /** ن5 — تبويبٌ جديد على رابطٍ (بسياسة المواقع) ثمّ الانتقالُ إليه. */
  async newTab(url: string): Promise<CdpTabInfo> {
    if (!this.#allowed(url)) throw new Error("Navigation blocked by saved site permissions")
    const created = (await this.#send("Target.createTarget", { url }, null)) as { targetId?: unknown }
    if (typeof created?.targetId !== "string") throw new Error("تعذّر فتحُ تبويبٍ جديد")
    await Bun.sleep(400)
    return this.switchTab(created.targetId)
  }

  /** ن5 — رجوعٌ/تقدّمٌ في تاريخ التبويب: بلا مدخلٍ في الاتّجاه يُقال «لا» ولا يُنتقل؛ والوجهةُ تخضع لسياسة المواقع. */
  async goHistory(direction: "back" | "forward"): Promise<CdpHistoryResult> {
    await this.#guardCurrentPage()
    const history = (await this.#send("Page.getNavigationHistory", {})) as { currentIndex?: number; entries?: { id: number; url: string; title?: string }[] }
    const entries = history?.entries ?? []
    const entry = entries[(history?.currentIndex ?? -1) + (direction === "back" ? -1 : 1)]
    if (entry === undefined) return { ok: false, why: direction === "back" ? "لا صفحةَ سابقة في تاريخ هذا التبويب" : "لا صفحةَ لاحقة في تاريخ هذا التبويب" }
    if (!this.#allowed(entry.url)) return { ok: false, why: "Navigation blocked by saved site permissions" }
    this.#console = []
    this.#network.clear()
    await this.#send("Page.navigateToHistoryEntry", { entryId: entry.id })
    await Bun.sleep(600)
    return { ok: true, url: entry.url, title: String(entry.title ?? "") }
  }
  ...
  /** S6 — رابطُ الصفحة الحاليّ كما يراه المتصفّح (بعد التحويلات)، لا كما طُلب. */
  async currentUrl(): Promise<string> {
    return this.#eval("location.href")
  }

  /** S6 — هل في الصفحة حقلُ كلمةِ سرّ؟ (النوعُ لا النصّ) — علامةُ «صفحة دخول» الثانية بعد الرابط. القيمةُ لا تُقرأ أبداً. */
  async hasPasswordField(): Promise<boolean> {
    return (await this.#eval("String(!!document.querySelector('input[type=password]'))")) === "true"
  }

  /** S6 — كعكاتُ أصلٍ بحقولها الكاملة كي تُستعاد كما كانت؛ من قناة الصفحة (تعمل من أيّ صفحة). */
  async cookiesFor(origin: string): Promise<readonly CdpCookie[]> {
    const result = (await this.#send("Network.getCookies", { urls: [origin] })) as { cookies?: unknown }
    if (!Array.isArray(result?.cookies)) return []
    return (result.cookies as Record<string, unknown>[]).filter((c) => typeof c.name === "string" && typeof c.value === "string").map((c) => ({
      ...
      ...(c.httpOnly === true ? { httpOnly: true } : {}), ...(c.secure === true ? { secure: true } : {}),
      ...(c.sameSite === "Strict" || c.sameSite === "Lax" || c.sameSite === "None" ? { sameSite: c.sameSite } : {}),
    }))
  }

  /** S6 — مخزنُ الأصل الحاليّ (localStorage) مفاتيحَ وقيماً — فارغٌ إن لم تكن الصفحةُ على ذلك الأصل. */
  async originStorage(origin: string): Promise<Readonly<Record<string, string>>> {
    const json = await this.#eval(`(() => { if (location.origin !== ${JSON.stringify(origin)}) return "{}"; const out = {}; try { for (let i = 0; i < localStorage.length; i++) { const k = localStorage.key(i); if (k !== null) out[k] = String(localStorage.getItem(k) ?? ""); } } catch {} return JSON.stringify(out) })()`)
    try { const parsed = JSON.parse(json) as unknown; return typeof parsed === "object" && parsed !== null ? (parsed as Record<string, string>) : {} } catch { return {} }
  }

  /** S6 — استعادةُ كعكاتٍ محفوظة (قبل التنقّل أو بعده — لا تحتاج صفحةَ الأصل). */
  async importCookies(cookies: readonly CdpCookie[]): Promise<number> {
    if (cookies.length === 0) return 0
    await this.#send("Network.setCookies", { cookies: cookies.map((c) => ({ ...c })) })
    return cookies.length
  }

  /** S6 — استعادةُ مخزن الأصل: لا تُكتب إلا والصفحةُ على الأصل نفسِه، ويعود عددُ المفاتيح المكتوبة. */
  async importOriginStorage(origin: string, storage: Readonly<Record<string, string>>): Promise<number> {
    const entries = Object.entries(storage)
    if (entries.length === 0) return 0
    const written = await this.#eval(`(() => { if (location.origin !== ${JSON.stringify(origin)}) return "0"; let n = 0; try { for (const [k, v] of ${JSON.stringify(entries)}) { localStorage.setItem(k, v); n++; } } catch {} return String(n) })()`)
    return Number.parseInt(written, 10) || 0
  }

  /** S6 — إعادةُ تحميل الصفحة الحاليّة (بعد استعادة جلسة). */
