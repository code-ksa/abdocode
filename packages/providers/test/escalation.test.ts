...
// 09-17 (مقيس على المثبَّت 4.0.45): 503 «Service temporarily overloaded» خمسَ مرّات فمات الدور — ازدحامُ المزوّد دليلٌ يُصعَّد عليه.
describe("ازدحامُ المزوّد دليلٌ مقيس", () => {
  const outage = (attemptId: string): EscalationEvidence => ({ kind: "provider_unavailable", detail: "إنفيديا (NIM): provider unavailable (HTTP 503)", attemptId })
  test("نموذجٌ خارج السلّم يبدأ من أوّل درجةٍ على الازدحام — وعلى البوّابة الحمراء يبقى خارجه", () => {
    const off = { ref: "nvidia/nvidia/nemotron-3-super-120b-a12b", spentAttempts: [] }
    const climbed = climb(off, LADDER, outage("outage:nvidia/nvidia/nemotron-3-super-120b-a12b"))
    expect(climbed.kind).toBe("escalated")
    if (climbed.kind === "escalated") { expect(climbed.to).toBe("ollama/qwen9b-gpu-32k:latest"); expect(receiptLine(climbed)).toContain("provider_unavailable") }
    expect(climb(off, LADDER, gate("a1")).kind).toBe("exhausted")
  })
  test("على السلّم يُصعَد درجةً، والدليلُ نفسُه لا يُصعَد به مرّتين، وسلّمٌ فارغ لا يُصعَد فيه", () => {
    const on = { ref: "ollama/qwen9b-gpu-32k:latest", spentAttempts: [] }
    const first = climb(on, LADDER, outage("outage:ollama/qwen9b-gpu-32k:latest"))
    expect(first.kind === "escalated" && first.to).toBe("qwen-coding-plan/qwen3-coder-plus")
    expect(climb(first.state, LADDER, outage("outage:ollama/qwen9b-gpu-32k:latest")).kind).toBe("already_spent")
    expect(climb({ ref: "x/y", spentAttempts: [] }, [], outage("outage:x/y")).kind).toBe("exhausted")
  })
})
