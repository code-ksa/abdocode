 ...
 * `provider_unavailable` (مقيس 09-17 على المثبَّت 4.0.45): المزوّدُ ردّ 503 «Service temporarily overloaded» خمسَ مرّاتٍ
 * فمات الدورُ في أوّله — ازدحامُ المزوّد دليلٌ مقيس كالبوّابة الحمراء، والصعودُ عليه إلى درجةٍ أخرى من سلّم المالك
 * (لا إلى نموذجٍ يُخترع) هو ما يبقي الدورَ حيّاً.
 */
export type EvidenceKind = "gate_failed" | "tool_refused" | "plan_rebutted" | "provider_unavailable"
...
export function climb(
  ...
  // ازدحامُ المزوّد: النموذجُ الحاليّ قد لا يكون على السلّم أصلاً (نموذجُ المالك الرئيس) — فيُبدأ من أوّل درجة؛
  // وعلى السلّم يُصعَد كالمعتاد. دليلُ البوّابة الحمراء يبقى كما كان: خارجَ السلّم لا صعود.
  const next = index < 0 ? (evidence.kind === "provider_unavailable" ? ladder[0] : undefined) : ladder[index + 1]
