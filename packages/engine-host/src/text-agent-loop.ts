...
export interface TextAgentLoopOptions {
  ...
  /**
   * S11 (2026-09-18) — الاستدعاءُ المكرَّر بوسائطه نفسها. الغياب = الإرث حرفياً: الحقبةُ تُقطع
   * «duplicate» فيفتح المضيف حقبةً بلا أداة ويعيد النموذجُ النداءَ نفسه (مقيس: 86 نقطةَ حفظٍ
   * بهذا السبب، وأدوارٌ تدور 5–25 دقيقة). الحضور = يُعاد إيصالُه السابق إلى النموذج بسطرٍ صريح
   * ويُعدّ، حتى `budget` مرّات (المضيف يحمل العدّ عبر حقب الدور) ثمّ «duplicate» كما كان.
   * ويُسمح بتنفيذه ثانيةً حين تغيّر جيلُ صفحة المتصفّح منذ التنفيذ السابق («الجيل N» في
   * الإيصالات) أو حين يكون فعلاً حاليّاً على نموذج (tap/fill/key/select) وقد نُفّذت أداةٌ أخرى
...
/** S11 — جيلُ صفحة المتصفّح كما تحمله إيصالاتُ الجسر والمتصفّح المملوك («الجيل N»)؛ آخرُ ذكرٍ يحكم. */
export const pageGenerationOf = (output: string): number | undefined => {
  let last: number | undefined
  for (const match of output.matchAll(/الجيل\s+(\d+)/gu)) last = Number(match[1])
  return last
}
 ...
 * باختبار (rail-policy.test يستورده). رُفع إلى 128 (09-16) ليتّسع لرفيعٍ بمئة جولة. */
export const MAX_TEXT_AGENT_ROUNDS = 128
...
export async function runTextAgentLoop(options: TextAgentLoopOptions): Promise<TextAgentLoopResult> {
  ...
  // S11 — إيصالاتُ كلّ الاستدعاءات (لا القراءاتُ وحدها) لإعادتها عند التكرار، وترتيبُ تنفيذها، وجيلُ الصفحة
  // لحظةَ تنفيذها. مطفأً (بلا الخيار) لا يُسجَّل شيء والمسارُ القديم بايتاً.
  ...
  const recordRun = (key: string, output: string): void => {
    if (!replayOn) return
    knownReceipts.set(key, output)
    lastRun.set(key, sequence)
    sequence += 1
    const generation = pageGenerationOf(output)
    if (generation !== undefined && generation > pageGeneration) pageGeneration = generation
    // جيلُ الصفحة **بعد** إيصال هذا الأمر: تنقّلُه هو لا يجعل تكرارَه «بعد تغيّر الجيل».
    pageGenAtRun.set(key, pageGeneration)
  }
  /** S11 — هل يجوز تنفيذُ المكرَّر ثانيةً؟ جيلُ الصفحة تغيّر منذ تنفيذه، أو فعلٌ حاليٌّ نُفّذت بعده أداةٌ أخرى. */
  const rerunAllowed = (command: string, key: string): boolean => {
    if (!replayOn) return false
    const at = pageGenAtRun.get(key)
    if (at !== undefined && pageGeneration > at) return true
    const last = lastRun.get(key)
    return STATEFUL_ACTION.test(command) && last !== undefined && sequence - 1 > last
  }
  ...
  /** S11 — إعادةُ الإيصال السابق للمكرَّر بسطرٍ صريح؛ `false` = لا إيصالَ معروف أو نفدت الميزانيّة ⇦ «duplicate» كما كان. */
  const replayDuplicate = async (command: string, key: string, tracked: Tracked | undefined): Promise<boolean> => {
    const previous = knownReceipts.get(key)
    if (!replayOn || previous === undefined || duplicateReplays >= replayBudget) return false
    duplicateReplays += 1
    recordAssistant()
    current = await followUp(
      `نتيجة «${command.split("\n", 1)[0]}» (إيصالٌ سابق — لم تُنفَّذ الأداة ثانيةً):\n${previous.slice(0, 14_000)}\n${DUPLICATE_REPLAY_LINE} (${duplicateReplays}/${replayBudget}).`,
      tracked,
    )
  ...
  for (let round = 0; round < maxRounds; round += 1) {
    ...
    if (parsed.kind === "batch") {
      ...
      const admitted = parsed.commands.filter((c) => options.volatile?.(c) === true || !seenCommandKeys.has(`${workspaceGeneration}:${c}`) || rerunAllowed(c, `${workspaceGeneration}:${c}`))
    ...
    if (seenCommandKeys.has(commandKey) && options.volatile?.(command) !== true && !rerunAllowed(command, commandKey)) {
