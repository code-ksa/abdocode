...
export function mountWorkspaceSettings(api){
  ...
  function frame(frame){
    ...
    // ن9 (09-16) — بابُ سياسة المواقع: طلبُ المحرّك («browser allow|block <نطاق>») يُحفظ هنا بالطريق الأصيل نفسه الذي يحفظ به زرُّ الإعدادات — تحقّقُ الشكل نفسُه، ثمّ workspace_store_set بتحقّق Rust.
    if(frame?.kind==='site-policy'){
      const site=String(frame.site||'').toLowerCase(),op=frame.op==='block'?'block':frame.op==='allow'?'allow':'';
      const preferences=api.state?.metadata?.preferences||{};
      if(!op||!/^([a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)*[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?$/u.test(site)){bridge.notice?.(L('Ignored an invalid site policy request.','تُجوهل طلبُ سياسة مواقع غيرُ صالح.'));return;}
      const current=[...new Set((preferences.blockedSites||[]).map(value=>String(value).toLowerCase()))];
      // allow: يُزال النطاقُ **والآباءُ الذين يحجبونه** (لا قائمةَ سماحٍ في المحرّك — الأبُ المحجوب يحجب أبناءه)، ويُسمّى المزالُ في الإشعار.
      const covering=op==='allow'?current.filter(value=>value!==site&&site.endsWith('.'+value)):[];
      const values=op==='block'?[...new Set([...current,site])]:current.filter(value=>value!==site&&!site.endsWith('.'+value));
      if(values.length>200){bridge.notice?.(L('Blocked sites list is full (200).','قائمةُ المواقع المحظورة ممتلئة (200).'));return;}
      if(op==='allow'&&(preferences.browserDefaultPermission||'allow')!=='allow')bridge.notice?.(L('The default site permission blocks every site; change it in Settings.','الإذنُ الافتراضيّ للمواقع يحظر كلَّ المواقع؛ غيّره من الإعدادات.'));
      blockedDraft=null;
      void savePreference('blockedSites',values).then(()=>bridge.notice?.((op==='block'?L('Site blocked: ','حُظر الموقع: '):L('Site allowed: ','سُمح بالموقع: '))+site+(covering.length?(L(' (also unblocked: ','（أُزيل حاجبُه أيضاً: ')+covering.join(', ')+')'):'')));
    }
