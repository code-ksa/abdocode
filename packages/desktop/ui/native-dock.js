...
export class NativeDock {
  ...
  register(id, node, options = {}) {
    ...
    if (!this.leaf(id) && !this.floating.includes(id)) {
      // (كان اللوحُ الجديد يُدسّ تبويباً في ورقة الطرفيّة إن وُجدت — فيرث شريطَها القصير.)
      const preferred=options.position||(id==='terminal'?'bottom':id==='browser'?'right':id==='changes'&&this.leaf('terminal')?'bottom':'right');
      const target=id==='changes'&&this.leaf('terminal')?'terminal':'conversation';
      this.move(id,this.leaf(target)?.id||this.leaves()[0]?.id,preferred,false,preferred==='left'||preferred==='right'?'root':'leaf');
    } else this.render();
  }
  ...
  open(id, position = 'right') {
    ...
    else this.move(id, this.leaves().find(l => l.tabs.includes('conversation'))?.id || this.leaves()[0]?.id, position, true, position==='left'||position==='right'?'root':'leaf');
    this.render();
  }
  close(id) {
  ...
  // الفتحُ البرمجيّ إلى يمين/يسار يقسم **الجذر** فيأخذ اللوحُ طولَ الصفحة كلَّه بعرضه المعتاد، والطرفيّةُ تبقى تحت المحادثة وحدها.
  // السحبُ بالفأرة إلى منطقة إفلات ورقةٍ يبقى نسبيّاً إلى تلك الورقة (scope='leaf').
  move(id, targetId, edge, record = true, scope = 'leaf') {
    ...
    if (scope === 'root' && this.tree && (edge === 'left' || edge === 'right') && id !== 'conversation') {
      const before = edge === 'left';
      // العرضُ المعتاد للوح الجانبيّ ≈ ٢٨٪ من المساحة — لا نصفُها كما في انقسام الورقة.
      this.tree = { id: `split-${Date.now()}-${++this.serial}`, axis: 'row', ratio: before ? .28 : .72, first: before ? fresh : this.tree, second: before ? this.tree : fresh };
      this.maximized = null; this.render(); return;
    }
