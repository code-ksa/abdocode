...
/// S6 — مجلّدُ ملفّ المتصفّح المملوك: `browser-profile` الدائم تحت ملفّ التطبيق، أو `browser-profile-session`
/// حين يختار المستخدمُ «لكلّ جلسة» صراحةً. لا مجلّدَ مؤقّتٌ ولا اسمٌ بالـpid.
fn browser_profile_dir(profile_root: &Path, shared: bool) -> PathBuf {
    profile_root.join(if shared { "browser-profile" } else { "browser-profile-session" })
}

#[cfg(test)]
mod browser_profile_tests {
    use super::browser_profile_dir;
    use std::path::Path;

    #[test]
    fn owned_browser_profile_is_persistent_under_the_app_profile_not_temp() {
        let root = Path::new("C:\\Users\\qa\\AppData\\Roaming\\io.abdocode.desktop");
        let shared = browser_profile_dir(root, true);
        let session = browser_profile_dir(root, false);
        assert_eq!(shared, root.join("browser-profile"));
        assert_eq!(session, root.join("browser-profile-session"));
        for path in [&shared, &session] {
            let text = path.to_string_lossy();
            assert!(text.starts_with(&*root.to_string_lossy()));
            assert!(!text.contains("Temp") && !text.contains(&std::process::id().to_string()));
        }
    }
}
...
fn browser_open(
    ...
    *slot = None;
    ...
    // لا في المجلّد المؤقّت ولا باسم pid: مقيسٌ أنّ كلَّ إعادةِ تشغيلٍ كانت خروجاً من كلّ حساب. تفضيلُ «لكلّ جلسة»
    // الصريح يبقى مُحترَماً: مجلّدٌ واحد يُمسح عند الإطلاق (لا مجلّداتٌ يتيمة بعدد التشغيلات).
    let profile = browser_profile_dir(&profile::directory(&app)?, workspace::browser_profile_shared(&app));
    if !workspace::browser_profile_shared(&app) {
